package ru.cheese.manager.configManager;
...
public final class ConfigManager implements IMinecraft {
    ...
    private final File autoCfgDir = new File(MinecraftClient.getInstance().runDirectory, "\\files\\configs\\AUTOCFG.cvc");
    ...
    public List<String> getAllConfigurations() {
        ...
        if (files != null) {
            for (File file : files) {
                if (file.isFile() && file.getName().endsWith(".cvc")) {
                    String configName = file.getName().substring(0, file.getName().lastIndexOf(".cvc"));
    ...
    public Config findConfig(String configName) {
        ...
        if (new File(CONFIG_DIR, configName + ".cvc").exists())
