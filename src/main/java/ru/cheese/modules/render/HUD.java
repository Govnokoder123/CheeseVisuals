...
import ru.cheese.mixin.iface.ItemCooldownEntryAccessor;
import ru.cheese.mixin.iface.ItemCooldownManagerAccessor;
import ru.cheese.modules.setting.*;
import ru.cheese.events.Event;
import ru.cheese.events.impl.EventUpdate;
import ru.cheese.events.impl.render.EventRender2D;
...
import ru.cheese.util.animations.Animation;
import ru.cheese.util.animations.impl.EaseBackIn;
import ru.cheese.util.color.ColorUtil;
import ru.cheese.manager.fontManager.FontUtils;
import ru.cheese.util.math.MathUtil;
import ru.cheese.util.player.ServerUtil;
import ru.cheese.util.render.RenderAddon;
import ru.cheese.util.render.RenderUtil;
import ru.cheese.util.render.Scissor;
...
import static ru.cheese.util.color.ColorUtil.hud_color;
import static ru.cheese.util.render.RenderUtil.*;
...
public class HUD extends Function {
    ...
    public final Dragging watermarkDrag = CheeseVisuals.getInstance().createDrag(this, "WaterMark", 10, 10);
    public final Dragging targethudDrag = CheeseVisuals.getInstance().createDrag(this, "TargetHUD", 10, 45);
    public final Dragging keybindsDrag = CheeseVisuals.getInstance().createDrag(this, "KeyBindsHUD", 10, 95);
    public final Dragging stafflistDrag = CheeseVisuals.getInstance().createDrag(this, "StaffListHUD", 10, 128);
    public final Dragging itemcooldownDrag = CheeseVisuals.getInstance().createDrag(this, "CoolDownHUD", 10, 165);
    public final Dragging potionhudDrag = CheeseVisuals.getInstance().createDrag(this, "PotionHUD", 10, 198);
    public final Dragging coordinateshudDrag = CheeseVisuals.getInstance().createDrag(this, "CoordinatesHUD", 10, 198);
    public final Dragging armorDrag = CheeseVisuals.getInstance().createDrag(this, "ArmorHUD", 478, 468);
    ...
    private void waterMark(EventRender2D render2D) {
        ...
        String textLogo = "CheeseVisuals";
        String textInfo = MinecraftClient.getInstance().getSession().getUsername() + " | " + ClientManager.getFps() + " FPS | " + ClientManager.getPing() + " MS | " + ClientManager.getBps(mc.player) + " BPS";
    ...
    public LivingEntity getTarget(LivingEntity nullTarget) {
        ...
        if (visibleCrosshair.get() && mc.crosshairTarget instanceof EntityHitResult) {
